...
Latent space analysis — PCA, geographic, and trajectory plots.

Usage:
    python latent_analysis.py --results_dir results/20260428_143012
"""
...
def plot_pca_variance(P, figures_dir):
    ...
    save(figures_dir, "01_pca_variance.png")


def plot_pca_by_lat(P, lats, figures_dir):
    ...
    save(figures_dir, "02_pca_latitude.png")


def plot_pca_by_lon(P, lons, figures_dir):
    ...
    save(figures_dir, "03_pca_longitude.png")


def plot_pca_by_season(P, ts, figures_dir):
    ...
    save(figures_dir, "04_pca_season.png")


def plot_geographic(lats, lons, device_ids, figures_dir):
    ...
    save(figures_dir, "05_geographic.png")


def plot_latent_trajectories(P, ts, device_ids, figures_dir, n_floats=6):
    ...
    for ax, did in zip(axes, top_devices):
        ...
        ax.scatter(pc[:, 0], pc[:, 1], c=t_norm, cmap="plasma", s=20, zorder=5, linewidths=0)
    ...
    plt.tight_layout()
    save(figures_dir, "06_latent_trajectories.png")


def plot_latent_dims_over_time(P, ts, figures_dir, n_dims=6):
    ...
    plt.tight_layout()
    save(figures_dir, "07_latent_dims_time.png")


def plot_umap(P, lats, lons, figures_dir):
    if not HAS_UMAP:
        return
    print("Plotting UMAP...")
    ...
    plt.tight_layout()
    save(figures_dir, "08_umap.png")


def main(results_dir):
    latent_path = os.path.join(results_dir, "latent_cycles.pt")
    figures_dir = os.path.join(results_dir, "figures")
    os.makedirs(figures_dir, exist_ok=True)

    all_records, _, _, _ = load_records(latent_path)
    P, lats, lons, ts, device_ids = records_to_arrays(all_records)

    print(f"\nLatent vectors: {P.shape}")
    print(f"Lat range: {lats.min():.1f} -> {lats.max():.1f}")
    print(f"Lon range: {lons.min():.1f} -> {lons.max():.1f}\n")

    plot_pca_variance(P, figures_dir)
    plot_pca_by_lat(P, lats, figures_dir)
    plot_pca_by_lon(P, lons, figures_dir)
    plot_pca_by_season(P, ts, figures_dir)
    plot_geographic(lats, lons, device_ids, figures_dir)
    plot_latent_trajectories(P, ts, device_ids, figures_dir)
    plot_latent_dims_over_time(P, ts, figures_dir)
    plot_umap(P, lats, lons, figures_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results_dir", type=str, required=True,
                        help="Timestamped results directory containing latent_cycles.pt")
