...
Runs the full data pipeline from raw NetCDF files to the interpolated CSV
that training reads. Only needs to be run once (or if the raw data changes).
...
# step 1 — convert netcdf to flat csv
...
def nc_to_csv(input_path, output_path):
    ...
    # precompute cast boundaries from the row_size arrays
    row_boundaries = {}
...
# step 2 — drift statistics (how far each float moves between casts)
...
# step 3 — drop floats that drift too much or have too few casts
...
# runs the full pipeline for one PFL dataset
...
def main():
    ...
    # save the concatenated interpolated data — this is what INTERP_PATH points to
